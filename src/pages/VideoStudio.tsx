...
const VideoStudio: React.FC = () => {
    ...
    /* ═════════════════════ RENDER ═════════════════════ */
    return (
        <div className="flex flex-col h-screen bg-[#0d0d0d] text-gray-200 overflow-hidden" style={{ fontFamily: "'JetBrains Mono', 'DM Mono', monospace" }}>
        ...
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 14px; height: 14px; border-radius: 50%; background: #7c3aed; cursor: pointer; border: 2px solid #a78bfa; }
      ...
      `}</style>
            ...
            {/* ═══════ TOP TOOLBAR ═══════ */}
            <div className="h-12 bg-[#141414] border-b border-white/5 flex items-center justify-between px-4 shrink-0 z-20">
                <div className="flex items-center gap-3">
                    <Film size={20} className="text-violet-400" />
                    <span className="font-bold text-sm tracking-wider text-white">VIDEO STUDIO</span>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => setShowEffects(!showEffects)}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${showEffects ? 'bg-violet-600 text-white' : 'bg-white/5 hover:bg-white/10 text-gray-400'}`}>
                        <Wand2 size={14} /> Effects
                    </button>
                    <button onClick={() => setFadeIn(!fadeIn)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${fadeIn ? 'bg-blue-600 text-white' : 'bg-white/5 hover:bg-white/10 text-gray-400'}`}>
                        Fade In
                    </button>
                    <button onClick={() => setFadeOut(!fadeOut)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${fadeOut ? 'bg-blue-600 text-white' : 'bg-white/5 hover:bg-white/10 text-gray-400'}`}>
                        Fade Out
                    </button>
                    <button onClick={() => { setExportOpen(true); exporter.resetExport(); }}
                        className="flex items-center gap-1.5 px-4 py-1.5 bg-violet-600 hover:bg-violet-700 rounded-lg text-xs font-bold transition-colors text-white">
            ...
            <div className="flex flex-1 overflow-hidden min-h-0">
                ...
                {/* ──── LEFT PANEL ──── */}
                <div className="w-72 bg-[#141414] border-r border-white/5 flex flex-col shrink-0">
                    {/* tabs */}
                    <div className="flex border-b border-white/5 shrink-0">
                        {([['media', 'My Media'], ['library', 'Library'], ['templates', 'Templates'], ['transitions', 'Transitions']] as const).map(([key, label]) => (
                            <button key={key} onClick={() => setLeftTab(key as LeftTab)}
                                className={`flex-1 py-2.5 text-[10px] font-bold tracking-wider transition-colors ${leftTab === key ? 'text-violet-400 border-b-2 border-violet-500' : 'text-gray-500 hover:text-gray-300'}`}>
                                {label}
                            </button>
                        ))}
                    </div>
                        ...
                        {leftTab === 'media' && (
                            <>
                                <button onClick={() => fileInputRef.current?.click()}
                                    className="w-full flex items-center justify-center gap-2 py-3 bg-violet-600/20 hover:bg-violet-600/30 text-violet-400 border border-violet-500/30 rounded-lg text-xs font-bold transition-colors">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                    {media.mediaItems.map(item => (
                                        <div key={item.id}
                                            ...
                                            className="group relative aspect-video rounded-lg overflow-hidden border border-white/10 cursor-grab hover:ring-2 ring-violet-500 transition-all bg-white/5">
                                            {item.thumbnailUrl ? (
                        ...
                        {leftTab === 'library' && (
                            <>
                                {library.hasApiKey ? (
                                    ...
                                    <>
                                        <div className="bg-amber-900/20 border border-amber-500/20 rounded-lg p-3 mb-2">
                                            <p className="text-[10px] text-amber-400 font-bold">No API Key</p>
                                            <p className="text-[9px] text-amber-400/70 mt-1">Set VITE_PIXABAY_API_KEY in .env to search. Showing sample assets below.</p>
                                        </div>
                                        <div className="grid grid-cols-2 gap-2">
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    {/* Effects bar */}
                    {showEffects && (
                        <div className="bg-[#1a1a1a] border-b border-white/5 px-4 py-3 flex gap-2 flex-wrap shrink-0">
                            {EFFECTS.map((fx, i) => (
                                <button key={i} onClick={() => setActiveEffect(i)}
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${activeEffect === i ? 'bg-violet-600 text-white' : 'bg-white/5 hover:bg-white/10 text-gray-400'}`}>
                                    {fx.name}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex-1 flex items-center justify-center bg-[#0a0a0a] relative overflow-hidden">
                        {activeVideoUrl ? (
                            <div className={`relative w-full max-w-3xl aspect-video ${fadeClass}`}>
                                ...
                                {!isPlaying && (
                                    <button onClick={togglePlay} className="absolute inset-0 flex items-center justify-center">
                                        <div className="w-16 h-16 bg-violet-600/80 rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-2xl">
                            ...
                            <div className="text-center space-y-4">
                                <Film size={56} className="mx-auto text-gray-700" />
                                <p className="text-gray-500 text-sm">Upload media or load a template to begin</p>
                                <button onClick={() => { setLeftTab('media'); fileInputRef.current?.click(); }}
                                    className="px-6 py-2.5 bg-violet-600 hover:bg-violet-700 rounded-lg text-xs font-bold transition-colors">
                    ...
                    {/* Player controls */}
                    <div className="bg-[#141414] border-t border-white/5 px-4 py-2 flex items-center gap-3 shrink-0">
                        <button onClick={togglePlay} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors">
                            {isPlaying ? <Pause size={18} /> : <Play size={18} />}
                        </button>
                        <span className="text-[10px] font-mono text-gray-500 w-20">{fmt(currentTime)} / {fmt(duration)}</span>
                        <input type="range" min={0} max={duration || 1} step={0.01} value={currentTime} onChange={seek} className="flex-1" />
                        <button onClick={() => setMuted(!muted)} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors">
                ...
                <div className="w-64 bg-[#141414] border-l border-white/5 flex flex-col shrink-0 overflow-y-auto">
                    <div className="flex border-b border-white/5 shrink-0">
                        {([['properties', 'Properties'], ['adjust', 'Adjust']] as const).map(([key, label]) => (
                            <button key={key} onClick={() => setRightTab(key as 'properties' | 'adjust')}
                                className={`flex-1 py-3 text-[10px] font-bold tracking-wider transition-colors ${rightTab === key ? 'text-violet-400 border-b-2 border-violet-500' : 'text-gray-500 hover:text-gray-300'}`}>
                                {label === 'Properties' ? <Eye size={12} className="inline mr-1" /> : <Sliders size={12} className="inline mr-1" />}
                                {label}
                            </button>
                        ))}
                    </div>
                            ...
                            {rightTab === 'properties' ? (
                                ...
                                <>
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <button onClick={() => resetAdjustments(selectedClip.id)} className="text-[10px] text-gray-500 hover:text-white flex items-center gap-1 bg-white/5 px-2 py-1 rounded">
                    ...
                    <div className="mt-auto p-3 border-t border-white/5 space-y-2 shrink-0">
                        <div className="flex justify-between text-[10px]">
                            <span className="text-gray-500">Global Effect</span>
                            <span className="text-blue-400 font-bold">{EFFECTS[activeEffect].name}</span>
                        </div>
                        <div className="flex justify-between text-[10px]">
                            <span className="text-gray-500">Clips</span>
                            <span className="text-violet-400 font-bold">{clips.length}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* ═══════ TIMELINE ═══════ */}
            <div className={`h-36 bg-[#111] border-t flex flex-col shrink-0 transition-colors ${dragOverTimeline ? 'border-violet-500 bg-violet-900/10' : 'border-white/5'}`}
                onDrop={handleTimelineDrop}
                onDragOver={handleTimelineDragOver}
                onDragLeave={() => setDragOverTimeline(false)}>
                <div className="h-7 bg-[#1a1a1a] border-b border-white/5 flex items-center px-3 gap-2 shrink-0">
                    <Scissors size={12} className="text-gray-500" />
                    <span className="text-[10px] font-bold text-gray-500 tracking-wider">TIMELINE</span>
                    <span className="text-[10px] text-gray-600 ml-auto">{clips.length} clip{clips.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="flex-1 flex items-center overflow-x-auto scrollbar-thin px-4 gap-0">
                    ...
                    {clips.map((clip, idx) => {
                        const w = Math.max(80, (clip.endTrim - clip.startTrim) * 18);
                        ...
                        return (
                            <React.Fragment key={clip.id}>
                                <div onClick={() => { setSelectedClipId(clip.id); if (clip.src) setActiveVideoUrl(clip.src); }}
                                    className={`relative h-16 rounded-lg flex flex-col justify-center px-3 cursor-pointer shrink-0 transition-all border-2 ${selectedClipId === clip.id ? 'border-violet-400 ring-1 ring-violet-500/50' : 'border-transparent hover:border-white/20'}`}
                                    style={{ width: w, background: clip.background || clip.color + '33' }}>
            ...
            {/* ═══════ EXPORT MODAL ═══════ */}
            {exportOpen && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm" onClick={() => setExportOpen(false)}>
                    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {/* ═══════ TOAST ═══════ */}
            {toast && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-violet-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-2xl z-[60] animate-fadeIn">
                    {toast}
                </div>
            )}
        </div>
